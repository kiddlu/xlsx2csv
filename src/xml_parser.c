...
/* Standard format mappings (from Python version) */
static const struct {
    int id;
    const char *format;
} standard_formats[] = {
...
/* Get standard format by ID */
static const char *get_standard_format(int id) {
    for (size_t i = 0; i < sizeof(standard_formats) / sizeof(standard_formats[0]); i++) {
        if (standard_formats[i].id == id) {
            return standard_formats[i].format;
        }
    }
    return NULL;
}
...
int parse_shared_strings(xlsx2csvConverter *conv) {
    ...
    for (xmlNodePtr si = root->children; si; si = si->next) {
        if (si->type == XML_ELEMENT_NODE && 
            xmlStrcmp(si->name, (const xmlChar *)"si") == 0) {
            count++;
        }
    }
...
int parse_worksheet(xlsx2csvConverter *conv, int sheet_index, FILE *outfile) {
    ...
    for (xmlNodePtr row = sheetData->children; row; row = row->next) {
        if (row->type != XML_ELEMENT_NODE || 
            xmlStrcmp(row->name, (const xmlChar *)"row") != 0) {
            continue;
        }
        ...
        for (xmlNodePtr cell = row->children; cell; cell = cell->next) {
            ...
            if (type_attr && xmlStrcmp(type_attr, (const xmlChar *)"inlineStr") == 0 && is_node) {
                ...
                if (content) {
                    value = format_cell_value((char *)content, 
                                            (char *)type_attr, 
                                            (char *)style_attr, 
                                            conv);
                    xmlFree(content);
